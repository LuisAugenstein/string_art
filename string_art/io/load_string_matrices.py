...
def load_string_matrices(n_pins: int, pin_side_length: float, string_thickness: float, high_res: int, low_res: float) -> tuple[csr_matrix, csr_matrix]:
    string_matrices_dir = f"{root_path}/data/string_matrices"
    config_dir = f'{string_matrices_dir}/{n_pins}_{pin_side_length}_{string_thickness}_{high_res}_{low_res}'
    ...
    if os.path.exists(config_dir) and all([os.path.exists(path) for path in matrix_paths]):
        ...
        high_res_matrix, low_res_matrix = precompute_string_matrices(n_pins, pin_side_length, string_thickness, high_res, low_res)
