...
def filter_lines_for_fabricatability(possible_lines: list[Line], pins: list[Pin], min_angle: float, thresh=1e-8) -> np.ndarray:
    """
    Parameters
    -
    ...
    Returns
    -
    fabricatable: np.shape([4*n_edges]) boolean mask indicating which strings are fabricatable
    """
    n_pins = len(pins)
    inflated_edges = get_edges(n_pins).repeat(4, axis=0)
    fabricatable = np.ones(inflated_edges.shape[0], dtype=bool)
    ...
    fabricatable[edge_angles - min_angle < thresh] = False
    return [line for i, line in enumerate(possible_lines) if fabricatable[i]]
