...
def precompute_string_matrices(n_pins: int, pin_side_length: float, string_thickness: float, high_res: int, low_res: float):
...
def strings_to_lower_resolution(strings: list[String], high_res: int, low_res: int) -> list[String]:
    ...
    for step, string in enumerate(strings):
        print(f'{step}/{len(strings)} high to low resolution')
