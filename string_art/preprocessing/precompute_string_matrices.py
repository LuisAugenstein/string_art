...
from string_art.preprocessing.filter_lines_for_fabricatability import filter_lines_for_fabricatability
...
def precompute_string_matrices(n_pins: int, pin_side_length: float, string_thickness: float, min_angle: float, high_res: int, low_res: float):
    ...
    fabricatable_lines = filter_lines_for_fabricatability(connection_lines, pins, min_angle)
    strings = lines_to_strings_in_positive_domain(fabricatable_lines, high_res)
