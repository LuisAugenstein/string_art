...
class ColorGradient:
    def __init__(self, minValue=0.0, maxValue=1.0):
        ...
        self.color = None
        self.set_color_gradient_type(GradientType.Jet)

    def set_color_gradient_type(self, gradient_type):
        if gradient_type == GradientType.Jet:
            self.set_gradient_type_jet()
        elif gradient_type == GradientType.Parula:
            self.set_gradient_type_parula()
        else:
            self.set_gradient_type_jet()

    def set_gradient_type_jet(self):
        self.color = np.array([
            [0.0, 0.0, 0.5, 0.0],
            [0.0, 0.0, 1.0, 0.125],
            [0.0, 0.5, 1.0, 0.250],
    ...
    def set_gradient_type_parula(self):
        v = parula_colors()
        n = np.linspace(0, 1, v.shape[0])
        self.color = np.column_stack((v, n))

    def color_at_value(self, val):
        val = (val - self.minValue) / self.span

        index_mask = np.zeros_like(val)
        residual_mask = np.ones_like(val, dtype=bool)

        for i in range(self.color.shape[0]):
            index_mask[residual_mask] = i
            residual_mask[residual_mask] = ~(val[residual_mask] <= self.color[i, 3] & residual_mask[residual_mask])
        ...
        curr_red = red[index_mask]
        curr_green = green[index_mask]
        curr_blue = blue[index_mask]
        curr_val = v[index_mask]

        prev_red = red[np.maximum(1, index_mask - 1)]
        prev_green = green[np.maximum(1, index_mask - 1)]
        prev_blue = blue[np.maximum(1, index_mask - 1)]
        prev_val = v[np.maximum(1, index_mask - 1)]

        value_diff = curr_val - prev_val

        lambda1 = np.zeros_like(value_diff)
        non_zero_ind = value_diff > 0
        lambda1[non_zero_ind] = (curr_val[non_zero_ind] - val[non_zero_ind]) / value_diff[non_zero_ind]
        lambda2 = 1 - lambda1

        r = lambda1 * prev_red + lambda2 * curr_red
        g = lambda1 * prev_green + lambda2 * curr_green
        b = lambda1 * prev_blue + lambda2 * curr_blue

        return np.column_stack((r, g, b))


def parula_colors() -> np.ndarray:
    return np.array([
